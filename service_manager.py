"""
OS-level service management for Biometric Sync.
...
SERVICE_NAME = "biometric-erpnext-sync"
...
def install_service() -> dict:
    """Install the sync service to run in the background and on boot.

    NOTE: This only *installs and enables* the service.  It does NOT start it
    immediately because the foreground Flask process is still holding port 5000.
...
def start_background_service() -> None:
    ...
    so the foreground process has released port 5000 by the time the
...
def _install_linux() -> dict:
    ...
    unit_content = f"""[Unit]
Description=ERPNext Biometric Sync Service
...
Restart=on-failure
RestartSec=10
Environment=BIOMETRIC_SYNC_SERVICE_MODE=background
Environment=BIOMETRIC_PORT={port}
...
TASK_NAME = "ERPNextBiometricSyncService"
