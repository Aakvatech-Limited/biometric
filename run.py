#!/usr/bin/env python3
"""
Biometric Sync — Auto-bootstrapping launcher.
...
def print_banner() -> None:
    ...
    print()
    print("=" * 52)
    print("   Biometric Sync — Auto Setup & Launch")
    print("=" * 52)
    print()
...
def install_dependencies() -> None:
    ...
    # Check if the key packages are importable from the venv to skip
    # unnecessary installs (pyzk imports as `zk`)
    check = subprocess.run(
        [
            PYTHON_BIN, "-c",
            "import flask, apscheduler, zk, requests, dotenv",
        ],
        capture_output=True,
    )
...
def launch_app() -> None:
    ...
    # Start Flask as a subprocess so the browser-opener can fire
    try:
